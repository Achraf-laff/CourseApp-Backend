...
// Function to simulate buying a course without payment
export const buyCourseWithoutPayment = async (req, res) => {
  const { userId } = req;
  const { courseId } = req.params;
  ...
  try {
    ...
    // Simulate purchase without payment
    await Purchase.create({ userId, courseId });
    res.status(201).json({
      message: "Course purchased successfully without payment",
      course,
    });
  } catch (error) {
    res.status(500).json({ errors: "Error in course buying" });
